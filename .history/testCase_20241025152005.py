...
for i in range(len(N_list)):
    N = N_list[i]
    M = M_list[i]
    C = np.array(C_list[0])  # Assuming we are only using the first set of C
    P = np.array(P_list[0])   # Assuming we are only using the first set of P
    D = np.array(D_list[0])   # Assuming we are only using the first set of D
