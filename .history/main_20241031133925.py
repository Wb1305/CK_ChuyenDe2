...
plt.subplot(2, 1, 1)  # 2 hàng, 1 cột, vị trí 1
...
# Biểu đồ số lượng sản phẩm sản xuất tại mỗi nhà máy
plt.subplot(2, 1, 2)  # 2 hàng, 1 cột, vị trí 2
bar_width = 0.35
index = np.arange(N)

for j in range(M):
    plt.bar(index + j * bar_width,
            X_value[:, j], bar_width, label=f"Sản phẩm {j+1}")
