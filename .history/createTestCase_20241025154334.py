...
with open(output_file, 'w') as f:
    # Chuyển đổi thành chuỗi JSON với định dạng
    formatted_data = json.dumps(testcases, indent=4)

    # Sửa đổi định dạng để các mảng nội bộ nằm trên cùng một dòng
    formatted_data = formatted_data.replace('\n    ', ' ').replace(
        '    ', ' ').replace(', ', ',')  # Loại bỏ khoảng trắng không cần thiết

    # Viết chuỗi JSON đã định dạng vào file
