...
# Dữ liệu đầu vào (số lượng nhà máy và sản phẩm, chi phí, năng lực và nhu cầu)
N = 3  # Số nhà máy
M = 2  # Số sản phẩm
...
constraints = [
    ...
    cp.sum(X, axis=0) >= D,   # Ràng buộc đáp ứng nhu cầu sản phẩm
]
...
# Kết quả
print("Tổng chi phí sản xuất tối ưu:", problem.value)
print("Số lượng sản phẩm sản xuất tại mỗi nhà máy:")
print(X.value)

# Tổng chi phí sản xuất theo các biến quyết định (tổng sản lượng sản xuất tại mỗi nhà máy)
total_production_per_factory = np.sum(X.value, axis=1)
