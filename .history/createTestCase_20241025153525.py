...
with open(output_file, 'w') as f:
    formatted_data = json.dump(testcases, f, indent=4)
    formatted_data = formatted_data.replace('\n        ', ' ').replace(
        ', ', ',').replace(' ', '')  # Clean up the formatting
    # Now we want to format it again to ensure inner arrays are on one line
    formatted_data = formatted_data.replace(
        '], [', '],[')  # Remove spaces between inner arrays
    print("done created!")
